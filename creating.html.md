...
If the `delete-binding` command is successful, it should output a exit code of 0. No other output is required. If the `delete-binding` command isn't implemented it should exit with code 10. If the command failed, it should return any non-zero exit code. Stdout and stderr from the command will be logged by the ODB.
